...
#define DEVICE_FILENAME "/dev/ledkey_led"
#define DEVICE_FILENAME "/dev/ledkey_key"
...
int main(int argc, char *argv[])
{
	...
	int ret;
	int num = 1;
	struct pollfd Events[2]; //내가배열할 fd를 2개 잡음
	char keyStr[80];
...
//  dev = open(DEVICE_FILENAME, O_RDWR | O_NONBLOCK);
	...
	fflush(stdin);// clear하겠다는 의미
	memset( Events, 0, sizeof(Events)); //fd0으로 초기화
	...
	while(1)
	{
		...
		if(Events[0].revents & POLLIN)  //stdin
		{
			...
			if(keyStr[0] == 'q')
				break;
			keyStr[strlen(keyStr)-1] = '\0'; //끝에null문자 제거
			printf("STDIN : %s\n",keyStr);
			buff = (char)atoi(keyStr);//문자를 숫자로 바꿈
			write(dev,&buff,sizeof(buff));
		}
